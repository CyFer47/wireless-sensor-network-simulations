...
const std::map<std::string, ScaleRule> kScaleRules = {
    ...
    {"S8", {3500, 140, 5, 950.0, 950.0}},
    {"S9", {4000, 160, 5, 1020.0, 1020.0}},
    {"S10", {4500, 180, 6, 1080.0, 1080.0}},
    {"S11", {5000, 200, 6, 1150.0, 1150.0}},
};
    ...
    if (inTrafficWindow)
    {
        for (auto& c : gClusters)
        {
            ...
            for (size_t i = 0; i < c.members.size(); ++i)
            {
                ...
                ApplyEnergy(memberId, 0.0008 * gState.loadMultiplier);
                ...
                gGlobalRawRx++;
                ApplyEnergy(c.currentChNodeId, 0.0005);
...
static void RecoveryTick()
{
    ...
    if (gState.healingId == "H1")
    {
        ...
        c.recovered = true;
        AddEvent(gState.failureTime + gState.recoveryDelay, "RECOVERY", "INFO", static_cast<int>(c.clusterId), static_cast<int>(replacement), "CH handover applied");
    }
    else if (gState.healingId == "H2")
    {
        c.recovered = true;
        c.linkQuality = 1.0;
        AddEvent(gState.failureTime + gState.recoveryDelay, "RECOVERY", "INFO", static_cast<int>(c.clusterId), static_cast<int>(c.currentChNodeId), "route switch / parent change applied");
    }
    else if (gState.healingId == "H3")
    {
        ...
        c.recovered = true;
        c.loadFactor = 0.5;
        AddEvent(gState.failureTime + gState.recoveryDelay, "RECOVERY", "INFO", static_cast<int>(c.clusterId), static_cast<int>(replacement), "load shedding applied");
    }
    else if (gState.healingId == "H4")
    {
        ...
        AddEvent(gState.failureTime + gState.recoveryDelay, "RECOVERY", "INFO", static_cast<int>(c.clusterId), static_cast<int>(replacement), "recovery rebalancing applied");
    }
    c.failed = false;
    c.linkQuality = 1.0;
...
int main(int argc, char* argv[])
{
    ...
    if (gState.enableFailure && gState.variant != "V1")
    {
        ...
        if (gState.enableRecovery && gState.variant == "V3")
        {
            Simulator::Schedule(Seconds(gState.failureTime + gState.recoveryDelay), &RecoveryTick);
        }
    }
    Simulator::Schedule(Seconds(gState.dashboardInterval), &DashboardTick);
