#!/usr/bin/env python3
"""Run Final Scale5000 S8 Stage C (64-run matched control-vs-healing comparison batch) with resumable progress."""
...
STATE_DEFAULT = REPO_ROOT / "outputs" / "s8_stagec_state.json"
QUARANTINE_DEFAULT = REPO_ROOT / "outputs" / "s8_stagec_quarantine.json"
LOG_DEFAULT = REPO_ROOT / "outputs" / "s8_stagec_batch.log"
SPEC_ROOT_DEFAULT = REPO_ROOT / "runspecs" / "generated" / "s8_stagec"
...
# Stage C matrix: 8 failure/healing cases × 2 architectures × 2 loads × 2 seeds = 64 rows
# F1/H0 + F1/H1, F2/H0 + F2/H2, F3/H0 + F3/H3, F4/H0 + F4/H4
S8_STAGEC_MATRIX: List[Dict[str, str | int]] = []

def build_stagec_matrix():
    """Build 64-row Stage C matrix: H0 control + active healing (H1,H2,H3,H4), F1–F4, A/B, L1/L2, seed01/seed02."""
    families_and_healing = [
        ("F1", "H0"),
        ("F1", "H1"),
        ("F2", "H0"),
        ("F2", "H2"),
    ...
    for family, healing in families_and_healing:
        for arch in architectures:
            for load in loads:
                for seed in seeds:
                    seed_suffix = f"seed{seed:02d}"
                    run_spec_id = f"{family}_{healing}_{arch}_S8_{load}_{seed_suffix}"
                    variant = "V2" if healing == "H0" else "V3"
                    S8_STAGEC_MATRIX.append({
...
def spec_for_row(row: Dict[str, str | int]) -> dict:
    ...
    failure_time = S8_RULE.failure_time_s  # failure injection always enabled for F1-F4
    recovery_delay = None if healing_id == "H0" else S8_RULE.recovery_delay_s
    recovery_enabled = healing_id != "H0"
    recovery_profile = "m7_profile" if recovery_enabled else "none"
    
    return {
        "schema_version": "runspec_v1",
        "run_spec_id": row["run_spec_id"],
        "description": f"Final Scale5000 Stage C {row['architecture']} {row['failure_family']}/{healing_id} (V{'2' if healing_id == 'H0' else '3'}) {row['load']} seed{seed:02d}",
        "phase": "phase1",
        "owner": "vmware-sim",
        "notes": "Final Scale5000 S8 Stage C matched control-vs-healing comparison",
        ...
        "failure_injection": {"enabled": True, "target": "cluster_ch"},
        ...
        "reserved_architecture_b": {
            ...
            "controller_profile": "bsbssp_phase1" if row["architecture"] == "B" and recovery_enabled else "reserved",
            "planned_status": "active" if row["architecture"] == "B" and recovery_enabled else "planned",
        },
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run Final Scale5000 S8 Stage C matched control-vs-healing comparison batch")
    ...
    append_log(log_file, f"[S8C] batch_start {now_utc()} total_runs={len(S8_STAGEC_MATRIX)}")
    if args.dry_run:
        append_log(log_file, "[S8C] dry-run only; no simulations will be launched")
        return 0
    ...
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            # Pre-scan DB for already-complete rows and reuse them
            for idx, row in enumerate(S8_STAGEC_MATRIX, start=1):
                run_spec_id = row["run_spec_id"]
            ...
            for idx, row in enumerate(S8_STAGEC_MATRIX, start=1):
                ...
                if run_spec_id in state and state[run_spec_id].get("status") == "ok":
                    skipped += 1
                    append_log(log_file, f"[S8C][{idx}/{len(S8_STAGEC_MATRIX)}] SKIP {run_spec_id} (already complete)")
                    continue
                if args.max_runs > 0 and processed >= args.max_runs:
                    break
                processed += 1
                append_log(log_file, f"[S8C][{idx}/{len(S8_STAGEC_MATRIX)}] START {run_spec_id} ({processed} of active batch)")
                ...
                try:
                    ...
                    if existing:
                        ...
                        write_json(state_file, state)
                        append_log(log_file, f"[S8C][{idx}/{len(S8_STAGEC_MATRIX)}] SKIP {run_spec_id} (complete in DB)")
                        continue
                    ...
                    db_info = find_existing_complete(cur, run_spec_id)
                    if not db_info:
                        raise RuntimeError(f"DB verification failed after import: {run_spec_id} not found as complete")
                    ...
                    write_json(state_file, state)
                    write_json(quarantine_file, quarantine)
                    append_log(log_file, f"[S8C][{idx}/{len(S8_STAGEC_MATRIX)}] OK {run_spec_id} run_id={db_info['run_id']} newest_run_id={newest_run_id} s8_count={s8_count}")
                    ...
                    write_json(state_file, state)
                    write_json(quarantine_file, quarantine)
                    append_log(log_file, f"[S8C][{idx}/{len(S8_STAGEC_MATRIX)}] FAIL {run_spec_id}: {exc}")
                    return 1

        append_log(log_file, f"[S8C] batch_end {now_utc()} processed={processed} skipped={skipped}")
    finally:
        conn.close()
