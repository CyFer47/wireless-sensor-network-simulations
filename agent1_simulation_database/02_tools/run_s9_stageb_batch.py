#!/usr/bin/env python3
"""Run Final Scale5000 S9 Stage B (32-run healing-family validation for S9=4000 nodes)
DB-aware: pre-scan DB for existing complete rows and reuse them.
Stage B covers F1-F4 with all healing families H1-H4, both seeds (seed01/seed02), all architectures/loads.
"""
...
STATE_DEFAULT = REPO_ROOT / "outputs" / "s9_stageb_state.json"
QUARANTINE_DEFAULT = REPO_ROOT / "outputs" / "s9_stageb_quarantine.json"
LOG_DEFAULT = REPO_ROOT / "outputs" / "s9_stageb_batch.log"
SPEC_ROOT_DEFAULT = REPO_ROOT / "runspecs" / "generated" / "s9_stageb"
...
def build_matrix():
    """Build 32-row Stage B matrix: F1-F4 × H1-H4 × A/B × L1/L2 × seed01/seed02"""
    # Group 1: F1/H1 healing family
    for arch in ['A', 'B']:
        for load in ['L1', 'L2']:
            for seed in [1, 2]:
                S9_MATRIX.append({
                    "run_spec_id": f"F1_H1_{arch}_S9_{load}_seed{seed:02d}",
                    "architecture": arch,
                    "failure_family": "F1",
                    "healing_id": "H1",
                    "variant": "V3",
    ...
    # Group 2: F2/H2 healing family
    for arch in ['A', 'B']:
        for load in ['L1', 'L2']:
            for seed in [1, 2]:
                S9_MATRIX.append({
                    "run_spec_id": f"F2_H2_{arch}_S9_{load}_seed{seed:02d}",
                    "architecture": arch,
                    "failure_family": "F2",
                    "healing_id": "H2",
                    "variant": "V3",
    ...
    # Group 3: F3/H3 healing family
    for arch in ['A', 'B']:
        for load in ['L1', 'L2']:
            for seed in [1, 2]:
                S9_MATRIX.append({
                    "run_spec_id": f"F3_H3_{arch}_S9_{load}_seed{seed:02d}",
                    "architecture": arch,
                    "failure_family": "F3",
                    "healing_id": "H3",
                    "variant": "V3",
    ...
    # Group 4: F4/H4 healing family
    for arch in ['A', 'B']:
        for load in ['L1', 'L2']:
            for seed in [1, 2]:
                S9_MATRIX.append({
                    "run_spec_id": f"F4_H4_{arch}_S9_{load}_seed{seed:02d}",
                    "architecture": arch,
                    "failure_family": "F4",
                    "healing_id": "H4",
                    "variant": "V3",
...
def spec_for_row(row: Dict[str, object]) -> dict:
    ...
    # standard timing values for Stage B
    timing = {
    ...
    return {
        ...
        "description": f"Final Scale5000 Stage B S9 {row['architecture']} {row['failure_family']}/{healing} {row['load']} seed{seed:02d}",
        "phase": "phase1",
...
def find_existing_complete(cur, external_run_id: str):
    """Query DB for existing complete run matching spec_id using LIKE pattern (importer adds timestamp)"""
    cur.execute(
...
def main():
    ...
    append_log(log_file, f"[S9B] batch_start {now_utc()} total_runs={len(S9_MATRIX)}")
    if args.dry_run:
        append_log(log_file, "[S9B] dry-run; no simulations launched")
    ...
    try:
        ...
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            # pre-scan: find all existing complete rows
            reused_count = 0
            ...
            append_log(log_file, f"[S9B] pre-scan: reused {reused_count}/{len(S9_MATRIX)} existing complete rows")

            if args.dry_run:
                append_log(log_file, "[S9B] dry-run complete")
                return 0
            ...
            for idx,row in enumerate(S9_MATRIX, start=1):
                ...
                if state.get(spec_id,{}).get('status')=='ok':
                    append_log(log_file, f"[S9B][{idx}/{len(S9_MATRIX)}] SKIP {spec_id} (already complete)")
                    continue
                append_log(log_file, f"[S9B][{idx}/{len(S9_MATRIX)}] START {spec_id}")
                existing = find_existing_complete(cur, spec_id)
                if existing:
                    ...
                    write_json(state_file, state)
                    append_log(log_file, f"[S9B][{idx}/{len(S9_MATRIX)}] SKIP {spec_id} (complete in DB)")
                    continue
                try:
                    ...
                    write_json(state_file, state)
                    new_count += 1
                    append_log(log_file, f"[S9B][{idx}/{len(S9_MATRIX)}] OK {spec_id} run_id={row_db['run_id']}")
                    ...
                    append_log(log_file, f"[S9B][{idx}/{len(S9_MATRIX)}] FAIL {spec_id}: {exc}")
                    return 1
            append_log(log_file, f"[S9B] batch_end {now_utc()} processed={len(S9_MATRIX)} new_runs={new_count} reused={reused_count}")
    finally:
        conn.close()
    return 0
